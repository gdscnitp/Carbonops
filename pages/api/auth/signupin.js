...
export default async function SignupIn (req, res) {
   ...
   try{
    if ((!req.body.email || !req.body.password || !req.body.contact) || !req.body.dob ) {
    ...
    const regUser =  await Individual.findOne({email:{$eq:req.body.email}})
    ...
    {
        const newAccount = await PendAcc({email:req.body.email,password:req.body.password ,contact:req.body.contact,dob:req.body.dob})
